...
def constituents_to_record(parse_tree):
	...
	for subtree in parse_tree.subtrees():
		...
		if len(
				tmp_tag_list
				) > 1 and tmp_tag_list[1] == "S":  # Case when we have 'PRP-S' or 'WP-S'
			label = tmp_tag_list[0] + "-" + tmp_tag_list[1]
		# Case when we have one of the strange punctions, such as round
		# brackets
		...
		if subtree.height() == 2:
			...
			if span not in CONTROL_MAPPING:
				CONTROL_MAPPING[span] = labels[random.randint(
					0, num_labels - 1
					)]
...
def process_ontonotes(ontonotes_reader):
	records = {}
	records['ner'], records['pos'], records['pos_control'], records['const'], records['coref'], records[
		'srl'] = [], [], [], [], [], []
	for sentence in ontonotes_reader:
		...
		if sentence.parse_tree is not None:
			pos_record, pos_control_record, const_record = constituents_to_record(
				sentence.parse_tree
				)
	...
	for record in records:
		...
		for target in record['targets']:
			...
			patterns.append(
				{
					"text": temp +
					        f"<|endoftext|>{label_dict[target['label']]}"
					}
				)
	return patterns

...
class PPTrainer(Trainer):
	...
	def evaluation_loop(
		...
		# losses/preds/labels on GPU/TPU (accumulated for
		...
		for step, inputs in enumerate(dataloader):
			...
			# Prediction step
			loss, accuracy, logits, labels = self.prediction_step(
			...
			if accuracy is not None:
				...
				accuracies_host = accuracies if accuracies_host is None else torch.cat(
					(accuracies_host, accuracies), dim=0
					)
			...
			if self.args.eval_accumulation_steps is not None and (
				...
				if accuracies_host is not None:
					...
					all_accuracies = accuracies if all_accuracies is None else np.concatenate(
						(all_accuracies, accuracies), axis=0
						)
		...
		# samplers has been rounded to a multiple of batch_size, so we
		# truncate.
	...
	def prediction_step(
		...
		# labels may be popped when computing the loss (label smoothing for
		# instance) so we grab them first.
		if has_labels:
				...
				if isinstance(outputs, dict):
					...
					logits = tuple(
						v for k, v in outputs.items(
							) if k not in ignore_keys + ["loss", "accuracy"]
						)
				else:
					accuracy = outputs[1]
					logits = outputs[2:]

...
class GPT2PreTrainedModel(PreTrainedModel):
	...
	def _init_weights(self, module):
		...
		try:
			if self.config.mod_randomized:
				if isinstance(module, (nn.Linear, Conv1D)):
					module.weight.data.normal_(mean=self.config.init_mean, std=self.config.init_std)
		...
		for name, module in self.named_modules():
			...
			if matched:
				...
				if isinstance(module, (nn.Linear, Conv1D)):
					...
					if module.bias is not None:
						bimodal_normal(x=module.bias.data, mu=head_mean, sigma=head_std)
				elif isinstance(module, nn.LayerNorm):
					bimodal_normal(x=module.weight.data, mu=head_mean, sigma=head_std)
					bimodal_normal(x=module.bias.data, mu=head_mean, sigma=head_std)
			else:
	...
	def forward(
		...
		if labels is not None:
			...
			if self.eval_acc:
				...
				selected_logits = lm_logits[:, :-1, :][
					indices[:, 1:]]  # we need to shift one position backward for logits

...
class DataTrainingArguments:
	...
	task: Optional[str] = field(
		default='ner', metadata={
			"help": "Tasks, one or more of {pos, const, coref, ner, srl}."
			}, )
...
def main():
	...
	if training_args.do_train:
		data_files["train"] = os.path.join(
			data_args.data_dir, data_args.task, 'train.json'
			)
	data_files["validation"] = os.path.join(
		data_args.data_dir, data_args.task, 'test.json'
		)
	...
	if model_args.config_name:
		...
		config = AutoConfig.from_pretrained(
			model_args.gpt2_name_or_path, **config_kwargs
			)
	else:
	...
	if model_args.tokenizer_name:
		...
		tokenizer = AutoTokenizer.from_pretrained(
			model_args.gpt2_name_or_path, **tokenizer_kwargs
			)
	else:
		raise ValueError(
	...
	if model_args.gpt2_name_or_path:
		...
		logger.info(
			f"Training new gpt2 from scratch - Total size={n_params / 2 ** 20:.2f}M params"
			)
	...
	if model_args.prefix_model_path:
		config = AutoConfig.from_pretrained(
			model_args.prefix_model_path,
			cache_dir=model_args.cache_dir
			)
		model = GPT2ForProbingViaPrompting.from_pretrained(
			model_args.prefix_model_path, config=config, gpt2=gpt2
			)
	else:
	...
	if model_args.do_prune:
		...
		if training_args.output_dir is not None:
			torch.save(
				...
				os.path.join(
					training_args.output_dir,
					"mask" + str(model_args.num_of_heads) + ".pt"
					)
				)
